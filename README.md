...
- 文本框统一使用固定在可视区底部、软键盘上方的临时编辑面板；输入实时回显到原画布，选中文本后可用颜色槽改色并通过选区锚点缩放。图形支持直线、矩形、椭圆和光滑曲线，完成后显示锚点，可直接拖动改变形状。
- 保存后的白板可通过图层笔按钮重新编辑；眼睛控制白底与笔迹整体显示或隐藏。只有点击普通图层的笔按钮切换编辑层时，正在编辑的白板才会保存当前修改并自动隐藏。
- 套索选区抬笔后按实际内容显示高对比度矩形、8 个缩放锚点和旋转柄，操作菜单立即出现。菜单中的“复制”会在原位置生成并选中重叠副本，“拷贝”写入可跨页面使用的插件剪贴板，“剪切”写入剪贴板并删除原对象，“粘贴”在当前可视区中央生成新对象；另支持删除、改色和截屏。手指长按文本可直接选中，Pencil 在文本或套索工具下长按文本可选中。
